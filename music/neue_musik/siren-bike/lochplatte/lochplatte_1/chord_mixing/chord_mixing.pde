...
public void settings() {
  ...
  size(int(300*mmTOpix), int(300*mmTOpix), PDF, "chord_mixing.pdf");
}
...
void draw() {
  ...
  float circle_abstand = 35.0/2.0;
 
  Table table;
  
  table = loadTable("chord_mixing.csv", "header");
  ...
  for (TableRow row : table.rows()) {
      ...
      if (LochNummer > 0) {println("Loch Nr " + LochNummer + " Winkel " + Winkel1 );}
    }
  }
