...
public void settings() {
  ...
  size(int(300*mmTOpix), int(300*mmTOpix), PDF, "beating.pdf");
}
...
void draw() {
  ...
  float hole_diameter = 3.5;//in mm
  float circle_diameter_start = 195;//in mm
  float circle_diameter = 0;
  float circle_abstand = 60.0/2.0;
 
  Table table;
  
  table = loadTable("unclear1.csv", "header");
  ...
  for (TableRow row : table.rows()) {
      ...
      if (LochNummer > 0) {println("Loch Nr " + LochNummer + " Winkel1 " + Winkel1 + " Winkel2 " + Winkel2);}
    }
  }
