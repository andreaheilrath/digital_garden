...
public void settings() {
  ...
  size(int(300*mmTOpix), int(300*mmTOpix), PDF, "shepard1.pdf");
}
...
void draw() {
  ...
  table = loadTable("shepard1.csv", "header");
  ...
  for (TableRow row : table.rows()) {
      ...
      int LochNummer2 = row.getInt(4 + data_set*columns_per_set);
      Float AbsLochgroesse2 = row.getFloat(5 + data_set*columns_per_set);
      Float WinkelAbstand2 = row.getFloat(7 + data_set*columns_per_set);
      ...
      if (Float.isNaN(WinkelAbstand2) == false){
        rotate(2*PI*WinkelAbstand2/100.0);
        ellipse(0, circle_diameter/2.0, AbsLochgroesse2, AbsLochgroesse2);
        popMatrix();
        pushMatrix();
        if (LochNummer2 > 0) {println("Loch Nr " + LochNummer2 + " Winkel Abstand " + WinkelAbstand2 + " Abs Lochgröße " + AbsLochgroesse2);}
      }      
    }
  }
