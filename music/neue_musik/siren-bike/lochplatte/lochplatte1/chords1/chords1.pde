...
public void settings() {
  ...
  size(int(300*mmTOpix), int(300*mmTOpix), PDF, "chords1.pdf");
}
...
void draw() {
  ...
  float circle_diameter_start = 295;//in mm
  float circle_diameter = 0;
  float circle_abstand = 60.0/2.0;
 
  Table table;
  
  table = loadTable("chords1.csv", "header");
  ...
  int columns_per_set = 5;
  int data_sets = no_columns / columns_per_set;
  
  println(data_sets + " data sets");
  
  for (int data_set = 0; data_set < data_sets; data_set += 1){
    circle_diameter = circle_diameter_start + circle_abstand*data_set - circle_abstand*data_sets;
    guide_circle(0,0,circle_diameter,circle_diameter);
    cut_mode();
    pushMatrix();
  for (TableRow row : table.rows()) {
      int LochNummer = row.getInt(0 + data_set*columns_per_set);
      Float Winkel1 = row.getFloat(1 + data_set*columns_per_set);
      Float Winkel2 = row.getFloat(2 + data_set*columns_per_set);
      Float Winkel3 = row.getFloat(3 + data_set*columns_per_set);
      
      if (Float.isNaN(Winkel1) == false){
        rotate(2*PI*Winkel1/100.0);
        ellipse(0, circle_diameter/2.0, hole_diameter, hole_diameter);
        popMatrix();
        pushMatrix();
      }  
      
      if (Float.isNaN(Winkel2) == false){
        rotate(2*PI*Winkel2/100.0);
        ellipse(0, circle_diameter/2.0, hole_diameter, hole_diameter);
        popMatrix();
        pushMatrix();
      }  
      
      if (Float.isNaN(Winkel3) == false){
        rotate(2*PI*Winkel3/100.0);
        ellipse(0, circle_diameter/2.0, hole_diameter, hole_diameter);
        popMatrix();
        pushMatrix();
      }  
      if (LochNummer > 0) {println("Loch Nr " + LochNummer + " Winkel1 " + Winkel1 + " Winkel2 " + Winkel2 + " Winkel3 " + Winkel3);}
    }
  }
