...
void draw() {
 ...
 // cut_regular_holes(hole_diameter, outer_cicle_diameter-3*66-33 , basis_anzahl*5);
  ...
  circle_diam = outer_cicle_diameter +48 - (3*60.0*2) - 60;
  cut_random_holes(hole_diameter, circle_diam);
...
void cut_sheppard(float hole_diameter, float circle_diameter, int rows){
    ...
    for(int row = 1; row< rows+1; row++){
      radius = circle_diameter/2 + 24 - (60.0*row);
      ...
      delta_phi = delta_0*exp(phi*log(pow(2.0, 2.0/3.0))/TWO_PI);
      rotate(delta_phi);
      phi += delta_phi;
      }
